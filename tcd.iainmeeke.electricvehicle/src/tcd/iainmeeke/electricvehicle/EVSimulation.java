...
public class EVSimulation {
    ...
    /*
     * TODO - figure out a way to plug in the car - give a demand curve based on time it will take to charge vs charging
     * rates vs desired charge time - maybe add ways for stepped desire charge time (by 10pm be at 20%, by 3am be at
     * 50%) - some way to update the currentCharge if it is CHARGING.....do I need a run method to thread it? or just do
     * it in EV.java - can I have different charge rates?
     * 
     */
    ...
    public EVSimulation(EVType car, FlexiblePowerContext context, Calendar homeTimeLower, Calendar homeTimeHigher,
        ...
        try {
            ...
            prop.load(input);
            runningOnPi = Boolean.parseBoolean(prop.getProperty("onPi"));
    ...
    /**
     * get the ratio of time left until needs to be charged to time it takes to charge. i.e 6 hours until the car needs
     * to be charged, takes 2 hours. This returns 3.
     * 
    ...
    /**
     * checks if the car should be plugged in (current time is past arrive home time and before desired charge time) and
     * updates pluggedIn accordingly Also updates how much the car is charging Also update the desiredChargeTime and
     * arriveHomeTime so they make sense
     * 
     * 
     * This method is called whenever a bid is made (through the isPluggedIn method) and whenever a priceUpdate happens
     * (through the setCharging method) This should keep everything accurate
     */
    private void updatePlugChargeStatus() {
        ...
        //update plugged in
        if (currentTimeMillis >= arriveHomeTime.getTimeInMillis()
                && currentTimeMillis <= desiredChargeTime.getTimeInMillis()) {
            pluggedIn = true;
        } else {
            pluggedIn = false;
        }
     ...
     * 
     * @param power
     *            boolean to set charging to. True is charging, as per final booleans above
     */
    ...
    private void changeGPIO(String pinName, int status) throws IOException {
        File valueFile = new File("/sys/class/gpio/" + pinName + "/value");
        FileWriter valueStream = new FileWriter(valueFile, false);
        String statusString = String.valueOf(status);

        valueStream.write(statusString);
        valueStream.close();
